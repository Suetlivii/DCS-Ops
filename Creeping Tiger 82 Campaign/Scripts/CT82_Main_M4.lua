...
if ME_isBlueCap == true then 
    ...
    MinhadHornet_CAPZone = ZONE:New( "MinhadHornet_CAPZoneName")
    --min alt, max alt, min patrol speed, max patrol speed, min engage speed, max engage speed, radio/baro alt
    BlueA2ADispatcher:SetSquadronCap( "MinhadHornet", MinhadHornet_CAPZone, 2000, 5000, 600, 800, 800, 1200, "RADIO" )
    --name, max spawned groups, min time sec, max time sec
    BlueA2ADispatcher:SetSquadronCapInterval( "MinhadHornet", 1, 5, 10, 1 )
